...
// Konek ke DB-B
const db = new sqlite3.Database("./DB-B.db", (err) => {
  if (err) console.error("MS3: Gagal koneksi DB-B", err);
  else console.log("MS3: Terhubung ke DB-B");
});

// --- BAGIAN PENTING: BUAT TABEL OTOMATIS ---
db.serialize(() => {
  db.run(`CREATE TABLE IF NOT EXISTS tbcarsweb (
...
app.listen(PORT, () => {
  console.log(`MS3 (Express) berjalan di http://localhost:${PORT}`);
});
