...
// MS2 juga konek ke DB-A (Shared Database dengan MS1)
const db = new sqlite3.Database("./DB-A.db", (err) => {
  if (err) console.error("MS2: Gagal koneksi DB-A", err);
  else console.log("MS2: Terhubung ke DB-A");
});
...
app.listen(PORT, () => {
  console.log(`MS2 (Express) berjalan di http://localhost:${PORT}`);
});
